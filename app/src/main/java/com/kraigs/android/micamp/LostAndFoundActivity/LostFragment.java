...
public class LostFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        lostRv = v.findViewById(R.id.lostRv);
        lostFb = v.findViewById(R.id.lostFb);
        lostRv.setLayoutManager(new GridLayoutManager(getActivity(),2));

        mAuth = FirebaseAuth.getInstance();
        currentUserID = mAuth.getCurrentUser().getUid();

        lostFb.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getActivity(),AddLostFoundActivity.class);
                intent.putExtra("missing_type","Lost");
                startActivity(intent);
            }
        });
        userCol = FirebaseFirestore.getInstance().collection("User");
        deleteRef = FirebaseFirestore.getInstance().collection(SharedPrefs.getCollege()).document("LostFound").collection("Lost");
